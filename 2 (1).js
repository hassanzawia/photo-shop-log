...
// Prevents the app from hanging on a weak/captive connection.
const NETWORK_TIMEOUT_MS = 4000;

// -----------------------------------------------------------------------------
// INSTALL — pre-cache app shell + vendor libraries.
// Each file is cached individually so ONE missing file (e.g. a missing icon)
// cannot abort the whole install and leave the app with no offline support.
// -----------------------------------------------------------------------------
...
self.addEventListener("fetch", (event) => {
  ...
  // 1) Only GET is cacheable. POST/PUT (Firebase writes) pass straight through.
  if (request.method !== "GET") return;

  const url = new URL(request.url);

  // 2) Never intercept Firestore / Firebase backend traffic.
  if (BYPASS_HOSTS.some((host) => url.hostname.endsWith(host))) return;

  // 3) Ignore non-http(s) schemes (chrome-extension:, data:, etc.).
  if (!url.protocol.startsWith("http")) return;

  event.respondWith(
    fetchWithTimeout(request)
      .then((networkResponse) => {
        // Cache same-origin files and cross-origin CDN libraries.
        if (
          networkResponse &&
          networkResponse.ok &&
      ...
      .catch(() =>
        ...
        caches.match(request).then((cached) => {
          ...
          // Only page navigations fall back to index.html. Returning HTML for a
          // missing script or image would corrupt the page, so those get a
          // clean error instead.
...
// Allow the page to trigger an immediate update: navigator.serviceWorker
